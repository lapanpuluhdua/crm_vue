    ...
    <div class="">
      <div class="card">
        <div class="card-header">
          ...
          <a v-if="$can(['Enquiry Create'])" href="#" @click="()=>{addModal=true;isAdding=true;clearForm()}" data-toggle="modal" class="btn btn-info btn-xs float-right">
        ...
        <div class="card-body pd-lg-25">
          <div class="table table-bordered table-striped m-0 responsive">
            <table class="table products-table">
              ...
              <tbody v-if="$can(['Enquiry View'])">
                <tr v-for="(item, i) in items" :key="i" v-if="items.length" >
                  <td>{{key=i+1}}</td>
                  <td>{{item.name}}</td>
                  <td>{{item.email}}</td>
                  <td>{{item.contactno}}</td>
                  <td style="max-width:250px; white-space: normal;word-wrap: break-word;">{{item.schoolname}}</td>
                  <td style="max-width:300px; white-space: normal;word-wrap: break-word;">{{item.enquirydetails}}</td>
                  ...
                  <td>
                    <div class="dropdown custom-dropdown">
                      ...
                      <div v-if="$can(['Enquiry Update', 'Enquiry Delete'])" class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        <button v-if="$can(['Enquiry Update'])"  @click="edit(item, i)" class="dropdown-item" data-toggle="modal"><i class="fa fa-edit"></i>&nbsp;&nbsp;Update</button>
                        <button v-if="$can(['Enquiry Delete'])" @click="showDeletingModal(item, i)" class="dropdown-item" data-toggle="modal"><i class="fa fa-trash"></i>&nbsp;&nbsp;Remove</button>
                      </div>
                    </div>
                  </td>
                </tr>
    ...
    <Modal
      ...
      <div slot="" class="pd-sm-20">
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <select  name="am" id="am" class="form-control" v-model="data.amdata">
                  <option v-for="amlist in amlists" :key="amlist.id" v-bind:value="{ amid: amlist.id, amname: amlist.name }">{{amlist.name}}</option>
                </select>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-group">
                <input v-model="data.status" type="text" name="status" class="form-control" placeholder="Enter status">
                <!-- <div class="text-danger" id="studNameError" data-ajax-feedback="student name"></div> -->
              </div>
            </div>
          </div>
...
<script>
  ...
  import { mapGetters } from 'vuex'
    ...
    async created(){
      ...
      const user = await this.callApi('get', 'users');
      
      if(res.status == 200){
        this.items = res.data
        this.amlists = user.data
